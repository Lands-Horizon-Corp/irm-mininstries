import { pgTable, integer, text, timestamp, unique, varchar, foreignKey, date, boolean, serial } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"



export const contactSubmissions = pgTable("contact_submissions", {
	id: integer().primaryKey().generatedByDefaultAsIdentity({ name: "contact_submissions_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	name: text().notNull(),
	email: text().notNull(),
...
export const churches = pgTable("churches", {
	id: integer().primaryKey().generatedByDefaultAsIdentity({ name: "churches_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	name: text().notNull(),
	...
	unique("churches_name_unique").on(table.name),
]);
...
export const ministerAwardsRecognitions = pgTable("minister_awards_recognitions", {
	id: serial().primaryKey().notNull(),
...
export const ministerEducationBackgrounds = pgTable("minister_education_backgrounds", {
	id: serial().primaryKey().notNull(),
	ministerId: integer("minister_id").notNull(),
...
export const ministerEmergencyContacts = pgTable("minister_emergency_contacts", {
	id: serial().primaryKey().notNull(),
	ministerId: integer("minister_id").notNull(),
	name: text().notNull(),
...
export const ministerEmploymentRecords = pgTable("minister_employment_records", {
	id: serial().primaryKey().notNull(),
	ministerId: integer("minister_id").notNull(),
...
export const ministerMinistryExperiences = pgTable("minister_ministry_experiences", {
	id: serial().primaryKey().notNull(),
	ministerId: integer("minister_id").notNull(),
...
export const ministryRanks = pgTable("ministry_ranks", {
	id: integer().primaryKey().generatedByDefaultAsIdentity({ name: "ministry_ranks_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	name: text().notNull(),
	description: text(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	unique("ministry_ranks_name_unique").on(table.name),
]);
...
export const ministrySkills = pgTable("ministry_skills", {
	id: integer().primaryKey().generatedByDefaultAsIdentity({ name: "ministry_skills_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	name: text().notNull(),
	description: text().notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	unique("ministry_skills_name_unique").on(table.name),
]);

export const ministerSeminarsConferences = pgTable("minister_seminars_conferences", {
	id: serial().primaryKey().notNull(),
