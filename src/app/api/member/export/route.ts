  ...
  try {
    ...
    const exportData = membersData.map((member) => {
      ...
      return {
        ...
        "Home Address": truncateText(member.homeAddress || "", 500), // Limit addresses to 500 chars
        ...
        Notes: truncateText(member.notes || "", 1000), // Limit notes to 1000 chars
    ...
    response.headers.set(
      "Content-Disposition",
      `attachment; filename="members-${new Date().toISOString().split("T")[0]}.xlsx"`
    );
