...
export default function MinistrySkillsTable() {
  ...
  const handleExportToExcel = async () => {
    try {
      setIsExporting(true);
      const response = await fetch("/api/ministry-skills/export", {
        method: "GET",
      });

      if (!response.ok) {
        throw new Error("Failed to export ministry skills");
      }
      ...
      link.href = url;
      link.download = `ministry-skills-${new Date().toISOString().split("T")[0]}.xlsx`;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center border-t pt-4">
        <Button
          className="gap-2"
          disabled={isExporting || ministrySkills.length === 0}
          variant="outline"
